...
const sendTicket = async () => {
  sendMessage.value.title = '線上醫療諮詢'
...
const sendReplyTicket = async (id: number) => {
  ...
  const res = await reply({
    id: store.tickets.filter((item) => { return item.title == '線上醫療諮詢' })[0].id,
...
<template>
  <div class="page">
    ...
    <div class="pages-user-message">
      <div class="message">
        <div class="msg-tit">
          <p>線上醫療諮詢: {{ PlayerStore?.playerInfo?.username }}您好</p>
        ...
        <div class="msg-box">
          ...
          <div class="box-item" v-if="store.tickets.length > 0" :class="item.role === 0 ? 'user' : 'bot'"
            v-for="(item, indexOfItem) in store.tickets.filter((item) => { return item.title == '線上醫療諮詢' })[0].thread"
            :key="indexOfItem">
            <div class="msg">
              <h5>
                {{
      ...
      <div class="msg-btn" v-if="store.tickets.filter((item) => { return item.title == '線上醫療諮詢' }) == 0">
